<div align="center">
  ...
  <img src="Frontend/public/logo.jpeg" alt="ATHENA Logo" width="140" style="border-radius: 24px; box-shadow: 0 12px 35px rgba(59, 130, 246, 0.45);" />
...
| **Pedagogical Goal** | Tells users *what* to believe | Passive answer retrieval | **Teaches users *how* to critically evaluate** |
...
- Translates complex analytical findings into digestible educational explanations of *why* cognitive biases and logical fallacies work.
...
flowchart TD
    ...
    classDef api fill:#0c4a6e,stroke:#38bdf8,stroke-width:2px,color:#ffffff;
    classDef engine fill:#4c1d95,stroke:#c084fc,stroke-width:2px,color:#ffffff;
    classDef storage fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#ffffff;

    subgraph ClientLayer ["Frontend Layer (Next.js 14 App Router + Tailwind CSS)"]
        UI["Landing Page (/)"]:::client
        Workspace["Investigation Workspace (/investigate)"]:::client
        Tools["Specialized Tools (Claims, Bias, Source, Trainer)"]:::client
        Profile["UNESCO MIL Profile & Leaderboard"]:::client
    end

    subgraph APILayer ["API Layer (FastAPI 2.0 Asynchronous Microservices)"]
        InvestigateRouter["/investigate (Full & Demo Pipelines)"]:::api
        ClaimsRouter["/claims/analyze"]:::api
        BiasRouter["/bias/detect"]:::api
        SourceRouter["/source/score"]:::api
        QuizRouter["/quiz/questions & submit"]:::api
        ForensicsRouter["/forensics/analyze"]:::api
    end

    subgraph OrchestrationLayer ["Intelligence Engine & AI Mesh"]
        AIProviderRouter{"Hybrid AI Provider Router"}:::engine
        GeminiAPI["Google Gemini 2.0 Flash API"]:::engine
        DeterministicEngine["Deterministic Pitch Demo Engine"]:::engine
        NewsAggregation["NewsAPI / GNews / Global RSS Feeds"]:::engine
    end

    subgraph StorageLayer ["Persistence & Edge Caching"]
        Supabase[("Supabase PostgreSQL (Auth & Profiles)")]:::storage
        Upstash[("Upstash Redis (Rate Limiting & Cache)")]:::storage
    end

    UI --> InvestigateRouter
    Workspace --> InvestigateRouter
    Tools --> ClaimsRouter & BiasRouter & SourceRouter & QuizRouter & ForensicsRouter
    Profile --> QuizRouter

    InvestigateRouter --> AIProviderRouter
    ClaimsRouter --> AIProviderRouter
    BiasRouter --> AIProviderRouter
    SourceRouter --> NewsAggregation

    AIProviderRouter -->|Live Key Present| GeminiAPI
    AIProviderRouter -->|Offline / Demo Pitch| DeterministicEngine

    InvestigateRouter --> Supabase
    QuizRouter --> Supabase
    InvestigateRouter --> Upstash
```
...
sequenceDiagram
    autonumber
    box rgba(99, 102, 241, 0.15) Client Layer
    actor User as 👨‍🎓 Youth Investigator
    participant NextJS as ⚡ Next.js 14 Client
    end

    box rgba(14, 165, 233, 0.15) API & Middleware Gateway
    participant FastAPI as 🚀 FastAPI Backend
    participant RateLimiter as 🛡️ Upstash Rate Limiter
    end

    box rgba(168, 85, 247, 0.15) AI & Verification Engine
    participant HybridEngine as 🧠 Hybrid AI Provider
    participant DB as 🗄️ Supabase Postgres
    end

    User->>NextJS: Submits viral claim text or news URL
    NextJS->>FastAPI: POST /investigate/full
    FastAPI->>RateLimiter: Check request quota (Sliding Window)
    RateLimiter-->>FastAPI: Rate limit OK

    par Parallel Epistemic Analysis
        FastAPI->>HybridEngine: Extract Claims & Evaluate Evidence
        FastAPI->>HybridEngine: Analyze Emotional Triggers & Bias
        FastAPI->>HybridEngine: Construct 4-Quadrant Perspectives
        FastAPI->>HybridEngine: Trace Narrative Mutation History
    end

    HybridEngine-->>FastAPI: Synthesized Investigation Payload
    FastAPI->>DB: Record investigation activity & award literacy XP
    FastAPI-->>NextJS: Return Trust Passport, Timeline & Quiz
    NextJS->>User: Renders Interactive 360° Investigation Workspace
    
    User->>NextJS: Solves Socratic Tutor Quiz Challenge
    NextJS->>FastAPI: POST /quiz/submit
    FastAPI->>DB: Update user MIL radar scores & grant badge
    DB-->>NextJS: Return updated level & leaderboard standing
    NextJS->>User: Displays celebratory badge & level up animation
```
...
%%{init: {'theme': 'dark'}}%%
flowchart LR
    classDef vercel fill:#1e1b4b,stroke:#818cf8,stroke-width:2px,color:#ffffff;
    classDef render fill:#042f2e,stroke:#14b8a6,stroke-width:2px,color:#ffffff;
    classDef db fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#ffffff;
    classDef ai fill:#4c1d95,stroke:#c084fc,stroke-width:2px,color:#ffffff;

    subgraph VercelEdge ["Vercel Edge Platform"]
        NextFrontend["Next.js 14 Web App<br/>(https://athena-eta-flame.vercel.app/)"]:::vercel
    end

    subgraph RenderCloud ["Render Cloud Platform"]
        FastAPIBackend["FastAPI ASGI Server<br/>(https://athena-backend-wrfq.onrender.com/)"]:::render
    end

    subgraph CloudPersistence ["Cloud Persistence & Caching"]
        SupabaseDB[("Supabase PostgreSQL<br/>Auth & Profiles")]:::db
        UpstashRedis[("Upstash Redis<br/>Rate Limiting")]:::db
    end

    subgraph AIProviders ["AI Provider Mesh"]
        GeminiFlash["Google Gemini 2.0 Flash"]:::ai
        PitchEngine["Deterministic Pitch Engine"]:::ai
    end

    NextFrontend -- REST API Calls --> FastAPIBackend
    FastAPIBackend -- Auth & Storage --> SupabaseDB
    FastAPIBackend -- Cache & Throttling --> UpstashRedis
    FastAPIBackend -- Epistemic Analysis --> GeminiFlash
    FastAPIBackend -- Failover / Pitch Demo --> PitchEngine
```
...
| 📜 **Open Source License** | **MIT License** *(100% Free & Open Source)* |
